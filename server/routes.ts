...
import { insertUserSchema, insertBookingSchema, insertAvailabilitySchema, insertBlockedDateSchema, insertAppointmentTypeSchema, insertAvailabilityPatternSchema, insertAppointmentTypeAvailabilitySchema, insertAvailabilityExceptionSchema, insertSubscriptionPlanSchema, insertUserSubscriptionSchema, insertBrandingSchema, insertFeedbackSchema, availabilitySettingsSchema, loginSchema, signupSchema, resendVerificationSchema, changePasswordSchema, changeEmailSchema, disconnectGoogleSchema, forgotPasswordSchema, resetPasswordSchema, checkoutStartSchema, validateCouponSchema } from "@shared/schema";
...
import { FeatureGate } from "@shared/featureGating";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Google OAuth start endpoint (redirect-based flow)
  ...
  app.get("/api/google-calendar/auth", async (req, res) => {
    try {
      ...
      const authUrl = googleCalendarService.getAuthUrl((req.session as any).user.id, req);
      res.json({ authUrl });
    } catch (error) {
  ...
  app.get("/api/google-calendar/status", async (req, res) => {
    try {
      ...
      const status = await googleCalendarService.getConnectionStatus((req.session as any).user.id);
      res.json(status);
    } catch (error) {
  ...
  app.post("/api/google-calendar/disconnect", async (req, res) => {
    try {
      ...
      await googleCalendarService.disconnect((req.session as any).user.id);
  ...
  app.get("/api/auth/me", async (req, res) => {
    try {
      ...
      res.json({ user: { id: user.id, email: user.email, name: user.name, picture: user.picture, isAdmin: user.isAdmin } });
    } catch (error) {
  ...
  app.post("/api/bookings",
    requireFeature("bookingLimit", {
      maxGetter: (f) => f.bookingLimit,
      usageCounter: countBookingsThisMonth,
      userIdGetter: (req) => {
        // For public bookings, get the business owner's userId from the request body
        const bookingData = req.body;
        return bookingData.userId || null;
      },
    }),
    async (req, res) => {
    try {
      ...
      if (businessUser && emailAppointmentType) {
        ...
        let branding = await storage.getBranding(businessUser.id);
        if (!branding) {
        ...
        googleCalendarService.createCalendarEvent(businessUser.id, {
  ...
  app.get("/api/public/booking/:token", async (req, res) => {
    try {
      ...
      const branding = await storage.getBrandingByUser(booking.userId);
      ...
      const publicBookingDetails = {
        ...
        business: {
          ...
          primaryColor: branding?.primaryColor || user.primaryColor,
          secondaryColor: branding?.secondaryColor || user.secondaryColor,
          accentColor: branding?.accentColor || user.accentColor,
          logo: branding?.logoUrl || user.logoUrl,
  ...
  app.put("/api/bookings/:id", async (req, res) => {
    try {
      ...
      try {
        ...
        const appointmentType = await storage.getAppointmentType(booking.appointmentTypeId);
        const branding = await storage.getBrandingByUser(booking.userId);
        const userFeatures = await storage.getUserFeatures(booking.userId);
  ...
  app.put("/api/users/:id", async (req, res) => {
    try {
      ...
      res.json({ message: "User updated successfully", user: { id: user.id, email: user.email, name: user.name, businessName: user.businessName, logoUrl: user.logoUrl, welcomeMessage: user.welcomeMessage, timezone: user.timezone, primaryColor: user.primaryColor, secondaryColor: user.secondaryColor, accentColor: user.accentColor } });
    } catch (error) {
  ...
  app.post("/api/appointment-types",
    requireFeature("appointmentTypeLimit", {
      maxGetter: (f) => f.appointmentTypeLimit,
      usageCounter: countAppointmentTypes,
    }),
    async (req, res) => {
    try {
  ...
  app.post("/api/admin/plans", requireAdmin, async (req, res) => {
    try {
      ...
      if (created.monthly || created.yearly) {
        await storage.updateSubscriptionPlan(upserted.id, {
      ...
      const plan = await storage.getSubscriptionPlan(upserted.id);
      res.json(plan);
    } catch (error) {
  ...
  app.patch("/api/admin/plans/:id", requireAdmin, async (req, res) => {
    try {
      ...
      if (priceMonthly !== undefined || priceYearly !== undefined) {
        ...
        if (created.monthly || created.yearly) {
          await storage.updateSubscriptionPlan(updated.id, {
  ...
  app.post("/api/branding", requireAuth, async (req, res) => {
    try {
      ...
      const { getUserFeatures } = await import("./lib/plan-features");
      const features = await getUserFeatures(storage, userId);
  ...
  app.post("/api/branding/logo", requireAuth, upload.single("file"), async (req, res) => {
    try {
      ...
      if (!(features.customBranding && features.logoUpload)) {
      ...
      await storage.updateBranding(userId, { logoUrl, updatedAt: new Date() });
  ...
  app.post("/api/branding/profile", requireAuth, upload.single("file"), async (req, res) => {
    try {
      ...
      await storage.updateBranding(userId, { profilePictureUrl, updatedAt: new Date() });
  ...
  app.post("/api/user-subscriptions", requireAuth, async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid user subscription data", error: error.errors });
      }
  ...
  app.patch("/api/user-subscriptions/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid update data", error: parseResult.error.errors });
      }
  ...
  app.post("/api/checkout/start", requireAuth, async (req, res) => {
    try {
      ...
      if (!priceId) {
        ...
        const prices = await ensureStripePrices(plan);
      ...
      if (!customerId) {
        ...
        if (sub) {
          ...
          sub = await storage.createUserSubscription({
            userId,
            planId,
    ...
    try {
      ...
      switch (event.type) {
        ...
        case "customer.subscription.canceled": {
          ...
          const userSub = await storage.getUserSubscriptionByStripeCustomerId(customerId);
          if (userSub) {
  ...
  async function upsertSubscriptionFromStripe(customerId: string, subscription: any) {
    try {
      ...
      const userSubscription = await storage.getUserSubscriptionByStripeCustomerId(customerId);
      if (!userSubscription) {
    ...
    try {
      ...
      const activeUsers = allUsers.filter(user => {
        const userBookings = allBookings.filter(booking => 
          booking.userId === user.id && new Date(booking.createdAt) > thirtyDaysAgo
        );
        ...
        if (subscription.status === "active") {
          ...
          if (plan && plan.price > 0) {
            ...
            if (subDate.getMonth() <= lastMonth && subDate.getFullYear() <= lastMonthYear) {
              monthlyRevenueLastMonth += plan.price;
            }
          }
        }
      }
      ...
      const planDistribution = allPlans.map(plan => {
        ...
        return {
          planName: plan.name,
          subscriptions,
          revenue: (subscriptions * (plan.price || 0)) / 100
        };
      });
  ...
  app.get("/api/admin/users", requireAdmin, async (req, res) => {
    try {
      ...
      const usersWithDetails = allUsers.map(user => {
        ...
        const userSubscription = allUserSubscriptions.find(sub => sub.userId === user.id);
        const userPlan = userSubscription ? allPlans.find(plan => plan.id === userSubscription.planId) : null;
        ...
        return {
          ...
          joinDate: user.createdAt instanceof Date ? user.createdAt.toLocaleDateString() : (user.createdAt ? new Date(user.createdAt).toLocaleDateString() : new Date().toLocaleDateString()),
  ...
  app.put("/api/availability-exceptions/:id", async (req, res) => {
    try {
      ...
      const exception = existingException.find(e => e.id === req.params.id);
  ...
  app.delete("/api/availability-exceptions/:id", async (req, res) => {
    try {
      ...
      const exception = existingExceptions.find(e => e.id === req.params.id);
      ...
      for (const plan of plans) {
        ...
        try {
          ...
          if (Object.keys(updates).length > 0) {
            await storage.updateSubscriptionPlan(plan.id, updates);
          }
  ...
  app.get("/api/notifications/unread-count", async (req, res) => {
    try {
      ...
      const count = await storage.getUnreadNotificationCount(session.userId);
      res.json({ count });
    } catch (error) {
