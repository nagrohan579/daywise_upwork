...
export function buildConfig({
  ...
  return {
    ...
    resolve: {
      alias: {
        assets: path.resolve(__dirname, "assets"), // Point to canva/assets, not root
    ...
    plugins: [
      new DefinePlugin({
        ...
        WEBAPP_FRONTEND_URL: JSON.stringify(process.env.WEBAPP_FRONTEND_URL || ''),
      }),
...
function buildDevConfig(options?: DevConfig): {
  ...
  let devServer: DevServerConfiguration = {
    server: enableHttps
      ...
      : "http",
    host,
    allowedHosts: "all", // Allow all hosts to fix "Invalid Host/Origin header" errors
    historyApiFallback: {
    ...
    static: {
      directory: path.resolve(__dirname, "assets"), // Point to canva/assets, not root
      publicPath: "/assets",
    },
  };
