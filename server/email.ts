...
// Smart environment detection - send real emails if we're on a live deployment
const isDevelopment = process.env.NODE_ENV === 'development';
const isLiveDeployment = process.env.REPLIT_DEV_DOMAIN || process.env.REPL_SLUG;

async function handleEmailSend(emailType: string, emailConfig: any): Promise<boolean> {
  try {
    // Send real emails if we're on a live deployment, even in development mode
    if (isDevelopment && !isLiveDeployment) {
      // Only log in local development (not live Replit deployments)
      console.log(`📧 [DEV MODE] ${emailType} Email:`, {
        from: emailConfig.from,
        to: emailConfig.to,
        subject: emailConfig.subject,
        html: emailConfig.html.substring(0, 200) + '...' // Log first 200 chars of HTML
      });
      return true;
    }
...
export async function sendVerificationEmail(email: string, name: string, verificationUrl: string): Promise<boolean> {
  const FROM_EMAIL = 'veilandvibe@gmail.com';
